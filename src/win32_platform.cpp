...
bool platform_create_window(int width, int height, const char* title)
{
    ...
    // fake window init OpenGl
    {
        ...
        if (!fakeRC)
        {
            SM_ASSERT(false, "Failed to create fake render context");
            return false;
        }

        if (!wglMakeCurrent(fakeDC, fakeRC))
        {
            SM_ASSERT(false, "Failed to make fake render context current");
            return false;
        }
        ...
        // cant use the same Device context bacause we already called "SetPixelFormat"
    ...
    // Actual OpenGl init
    {
        ...
        HGLRC rc = wglCreateContextAttribsARB(dc, 0, contextAtribs);
        if (!rc)
        {
        ...
        {
            SM_ASSERT(false, "Failed to make render context current");
            return false;
        }
    }
...
void* platform_load_gl_function(char* funName)
{
    ...
    if (!proc)
    {
        ...
        if (!proc)
        {
            SM_ASSERT(false, "Failed to load function %s", funName);
            return nullptr;
        }
    }
